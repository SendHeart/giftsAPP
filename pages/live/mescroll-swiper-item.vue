<template>
	<view>
		<mescroll-uni top="90" :fixed="false" :down="downOption" @down="downCallback" :up="upOption" @up="upCallback" @emptyclick="emptyClick" @scroll="scroll" @topclick="goTop" @init="mescrollInit">
...
<script>
	...
	export default {
		...
		watch:{
			// 监听下标的变化
			activeIndex(val){
				//console.log('watch:', this.i,' activeIndex:',this.activeIndex);  
				if (this.i === val && !this.isInit) {
					this.isInit = true; // 标记为true
					this.mescroll.triggerDownScroll();
					this.mescroll.scrollTo(0, 0)
					console.log('watch isInit:', this.isInit,' activeIndex:',this.activeIndex);  
				}else if (this.i === this.activeIndex) {
					this.page =  1
		...
		methods: {
			...
			upCallback(mescroll) {
				...
				this.getListDataFromNet(mescroll.num, mescroll.size, (curPageData)=>{
					...
					console.log("i="+this.i+", mescroll.num=" + mescroll.num + ", mescroll.size=" + mescroll.size + ", curPageData.length=" + curPageData.length,curPageData);
					...
					console.log('pdList:',JSON.stringify(this.pdList))
				}, () => {
					//联网失败的回调,隐藏下拉刷新的状态
					mescroll.endErr();
				})
			...
			* */
			getListDataFromNet(pageNum,pageSize,successCallback,errorCallback) {
				...
				//console.log('activeIndex:', activeIndex, 'live_type:', that.tab,' live_type_value:',that.tab_value );
				if(page > all_rows && page>1) {
					console.log('加载完成 page:', page, 'all_rows:',all_rows,' live_type_value:',that.tab_value);
