<template>
<view>
	<view v-if="hiddenmodalput" class="subject"> 
		<text class="toptitle">{{coupon_type==1?'优惠券':''}}{{coupon_type==2?'红包':''}}{{coupon_type==3?'积分奖励':''}}已经生成</text>
		<text>点击送出发送给对方</text>
	</view>
	<view class="uni-list"  v-if="!hiddenmodalput">
		<radio-group @change="confirm">
			<label class="uni-list-cell uni-list-cell-pd" v-for="(item, index) in coupons_info" :key="index">
				<view>
					<radio :value="item.id" :checked="index === current" />
				</view>
				<view>{{item.name}} {{item['start_time']}}至{{item['end_time']}}</view>
			</label>
		</radio-group>
	</view>
	 
	<!--
	 <view class="cu-modal" :class="!hiddenmodalput?'show':''">
	 	<view class="cu-dialog" @tap.stop="">
	 		<radio-group class="block" @change="confirm">
	 			<view class="cu-list menu text-left">
	 				<view class="cu-item" v-for="(item,index) in coupons_info" :key="index">
	 					<label class="flex justify-between align-center flex-sub">
	 						<view class="flex-sub">{{item['name']}} {{item['start_time']}}至{{item['end_time']}}</view>
	...
	-->
	<view v-if="hiddenmodalput" class="order-item">
		<view class="order-item-banner">
			<view class="gifts-item" :style="'background-image:url(' + coupon_img + ');background-repeat:no-repeat; background-size:100% 100%;-moz-background-size:100% 100%;'">
				...
				<text v-if="coupon_type==2" class="coupon-content">￥{{coupon_amount}}元</text>
				<text v-if="coupon_type==3" class="coupon-content">积分:{{coupon_amount}}</text>
				<text class="coupon-footer">{{coupon_footer}}</text>
				<text class="coupon-dueday">{{start_time?'(期限:'+start_time+'至':''}}{{end_time?end_time+')':''}}</text>
			</view>
			<view class="sentbtn">
				<button open-type="share"   @tap="shareToWXminiProgram" :hidden="(send_status==0?false:true)">送出</button>
				<button  @tap="qrcodeTapTag" :hidden="((send_status==0&&nums==0)?false:true)">二维码</button>
...
export default {
  ...
  methods: {
	shareToWXminiProgram: function () { //分享到微信小程序
		...
		var imageUrl = that.coupon_share_img;  
    ...
    qrcodeTapTag: function (e) {
      ...
      console.log('qrcodeTapTag coupons_json:', coupons_json,' share_coupon_qrcode',share_coupon_qrcode);
      that.setData({
