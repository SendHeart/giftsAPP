<template>
<view>
<scroll-view scroll-y  class="container" :style="(modalHiddenCoupon?'position:fixed;':'') +'height:'+dkheight+'px;'">
	<!-- 地址 -->
...
export default {
  ...
  methods: {
    ...
    confirmOrder: function () {
		...
		var token = uni.getStorageSync('token') ? uni.getStorageSync('token') : '1'
		var shop_type = getApp().globalData.shop_type;
		var order_num = that.order_num>0?that.order_num:1
    ...
    bindSelectAll: function () {
		...
		modalHiddenCoupon = !modalHiddenCoupon;
		that.selectedAllStatus = selectedAllStatus
		that.modalHiddenCoupon = modalHiddenCoupon
		that.couponType = 1 //优惠券
		console.log('selectedAllStatus :', selectedAllStatus, 'page_num:', page_num)
		if (selectedAllStatus && page_num == 0) {
			that.query_coupon()
		} else {
			that.coupons_list = that.coupons_quan_list
		}
    },
    ...
    bindSelectCoupon: function (e) {
		...
		if (coupon_type == 1) {
			...
			that.oupons_red_list = coupons_list
