...
(function ExportLibrary(root, factory) {
	...
	var PahoMQTT = (function(global) {
		...
		var Pinger = function(client, window, keepAliveInterval) {
			...
			var doPing = function() {
				if (!this.isReset) {
					...
					// this._client.socket.send(pingReq); lm
					plussocket.sendSocketMessage({
						data: pingReq,
						success: function() {
							// 
						},
			...
			this.reset = function() {
				this.isReset = true;
				// this._window.clearTimeout(this.timeout); lm
		...
		ClientImpl.prototype._doConnect = function(wsurl) {
			...
			plussocket.connectSocket({
				url: wsurl,
				protocols: ['mqtt'],
				complete: () => {}
			});

			plussocket.onSocketOpen(scope(this._on_socket_open, this))
			plussocket.onSocketMessage(scope(this._on_socket_message, this))
			plussocket.onSocketError(scope(this._on_socket_error, this))
			plussocket.onSocketClose(scope(this._on_socket_close, this))
			...
			this._connectTimeout = new Timeout(this, window, this.connectOptions.timeout, this._disconnected, [ERROR.CONNECT_TIMEOUT
				.code, format(ERROR.CONNECT_TIMEOUT)
			]);
		};
		...
		ClientImpl.prototype.store = function(prefix, wireMessage) {
			...
			uni.setStorageSync(prefix + this._localKey + wireMessage.messageIdentifier, JSON.stringify(storedMessage));
		};

		ClientImpl.prototype.restore = function(key) {
			var value = uni.getStorageSync(key); //localStorage.getItem(key);
			...
			try {
				switch (wireMessage.type) {
					case MESSAGE_TYPE.CONNACK:
						...
						if (this.connectOptions.cleanSession) {
							for (var key in this._sentMessages) {
								...
								uni.setStorageSync("Sent:" + this._localKey + sentMessage.messageIdentifier);
							}
							this._sentMessages = {};
							...
							for (var key in this._receivedMessages) {
								...
								uni.setStorageSync("Received:" + this._localKey + receivedMessage.messageIdentifier);
							}
							this._receivedMessages = {};
						}
					...
					case MESSAGE_TYPE.PUBREL:
						...
						uni.removeStorageSync("Received:" + this._localKey + wireMessage.messageIdentifier);
					...
					case MESSAGE_TYPE.DISCONNECT:
						...
						this._disconnected(ERROR.INVALID_MQTT_MESSAGE_TYPE.code, format(ERROR.INVALID_MQTT_MESSAGE_TYPE, [wireMessage
							.type
						]));
						break;
